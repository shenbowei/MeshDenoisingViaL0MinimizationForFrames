...
{
public:
    enum Type{kQVariant = -1, kBool = 0, kInt = 1, kDouble = 2, kQStringList = 3, kFileName = 4};
    ...
    Parameter(const QString &_defaultValue) : value_(QVariant(_defaultValue)), default_value_(QVariant(_defaultValue)), index_(0), default_index_(0), has_validator_(false), type_(kFileName) {}
    ~Parameter();
...
public:
    ...
    void setValue(const QString &_value) {value_ = (QVariant)_value;}
    ...
    QString getValueFileName() const {return value_.toString();}
    ...
    void print() {
        if(type_==kQStringList){
            qDebug()<<QString::number(index_);
        }else
            qDebug()<<value_.toString();
    }
...
public:
    ...
    bool getValue(const QString &_name, QString &_value) const;
    void setValue(const QString &_name, const QString &_value);
    ...
    void addParameter(const QString &_name, QString &_defaultValue, const QString &_label = "", const QString &_toolTip = "");
