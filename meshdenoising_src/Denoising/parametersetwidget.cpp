...
//        VBoxLayout->addWidget(ComboBox);
    ...
    else if(parameter_->getType() == Parameter::kFileName){
        QLineEdit *LineEdit = new QLineEdit;
        LineEdit->setText(parameter_->getValueFileName());
        widget_ = LineEdit;
        widget_type_ = kFile;
    }
}
...
void ParameterWidget::resetParameter()
{
    ...
    else if(parameter_->getType() == Parameter::kQStringList)
    {
        ...
        QLineEdit* LineEdit = dynamic_cast<QLineEdit *>(widget_);
        LineEdit->setText(parameter_->getDefaultValue().toString());
    }
    update();
}
...
void ParameterWidget::updateParameter()
{
    ...
    else if(parameter_->getType() == Parameter::kQStringList)
    {
        ...
        QLineEdit* LineEdit = dynamic_cast<QLineEdit *>(widget_);
        parameter_->setValue(LineEdit->text());
    }
    update();
}

void ParameterWidget::onParameterWidget()
{
    if(parameter_->getType() == Parameter::kFileName){
        QString filename = QFileDialog::getOpenFileName(this, tr("Choose Mesh"), "../Models", tr(" *.obj *.off *.ply"));
...
//        VBoxLayout_Setting_->addWidget(parameter_widget);
        ...
        if(parameter_widget->widget_type_ == ParameterWidget::kFile){
            QHBoxLayout *hLayout = new QHBoxLayout();
            QPushButton *chooseFile = new QPushButton(tr("..."), this);
            connect(chooseFile, SIGNAL(clicked()), parameter_widget, SLOT(onParameterWidget()));
            hLayout->addWidget(parameter_widget->getWidget());
            hLayout->addWidget(chooseFile);
            VBoxLayout_Setting_->addLayout(hLayout);
        }
        else
        {
