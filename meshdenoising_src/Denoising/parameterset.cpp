...
//file name
bool ParameterSet::getValue(const QString &_name, QString &_value) const
{
    if(!parameter_dictionary_.contains(_name)){
        _value = QString();
        return false;
        ...
        Parameter *p = parameter_dictionary_.value(_name);
        if(p->getType() != Parameter::kFileName){
            _value = QString();
            return false;
        }
        _value = p->getValueFileName();
        return true;
    }
}

void ParameterSet::setValue(const QString &_name, const QString &_value)
{
    if(!parameter_dictionary_.contains(_name))
        return;
    else{
...
void ParameterSet::addParameter(const QString &_name, QString &_defaultValue, const QString &_label, const QString &_toolTip)
{
    if(!parameter_dictionary_.contains(_name)){
        Parameter *p = new Parameter(_defaultValue);
        addParameter(_name, p, _label, _toolTip);
...
void ParameterSet::print()
{
    ...
    for(iter=parameter_dictionary_.begin();iter!=parameter_dictionary_.end();++iter){
        qDebug()<<iter.key()<<":";
        iter.value()->print();
    }
}
